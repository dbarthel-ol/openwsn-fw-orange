   ...
   for (i=0;i<MAXNUMNEIGHBORS;i++) {
      if (isThisRowMatching(l2_src,i)) {
         ...
         if (joinPrioPresent==TRUE){
            ...
            observer_property_update_uint8(PROPERTY_L3_LINK_PARENT_JOIN_PRIORITY, joinPrio);
         }
   ...
   if (isNeighbor(address)==FALSE) {
      ...
      while(i<MAXNUMNEIGHBORS) {
         if (neighbors_vars.neighbors[i].used==FALSE) {
            ...
            owsn_observer_link_l3_add(&(neighbors_vars.neighbors[i]));
            break;
         }
         i++;
      }
...
void removeNeighbor(uint8_t neighborIndex) {
   ...
   if(neighbors_vars.neighbors[neighborIndex].used){
      owsn_observer_link_l3_remove(&(neighbors_vars.neighbors[neighborIndex]));
   }
