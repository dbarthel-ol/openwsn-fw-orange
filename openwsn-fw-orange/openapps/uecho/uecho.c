...
void uecho_receive(OpenQueueEntry_t* request) {
   ...
   owsn_observer_frame_consume(request);
   ...
   //observer report
   owsn_observer_frame_produce(reply, 2);
   observer_property_declaration_uint16(PROPERTY_L4_DESTINATION_PORT, "UDP port", PREFIX_NONE, UNIT_NONE, reply->l4_destination_port);
   observer_property_declaration_byte_array(PROPERTY_L3_FRAME_DESTINATION_ADDRESS, "IPv6 destination address", 16, reply->l3_destinationAdd.addr_128b);
